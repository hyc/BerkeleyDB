...
// Needed to check for TLS support.
#include "db_config.h"
#include "db_cxx.h"
...
// Thread local storage modifier macros. Users need to #define one of
// these macros to use the right TLS modifier.
#ifndef HAVE_PTHREAD_TLS
#ifndef TLS_MODIFIER
#if defined(WIN32) || defined(DB_WIN32) || defined(DB_WINCE)
#define TLS_MODIFIER  __declspec( thread )
#elif defined(HAVE_TLS) && defined(HPTru64)
 ...
 * \defgroup dbstl_glob_int_func dbstl global functions for internal use only
 * @{
 */
