...
#include "db_map.h"
#include "internal/dbc.h"
#include "internal/db_container.h"
...
/// database handles of DB_BTREE or DB_HASH type, you will be using an 
/// equivalent of std::set or hash_set. This container stores the key in the
/// key element of a key/data pair in underlying database, and keeps the data
/// element empty;
...
public:
	...
	/// does not rely on key order, it is doing a set equal comparison---
...
/// will be using an equivalent of std::multiset or hash_multiset respectively.
/// This container stores the key in the key element of a key/data pair in 
/// underlying database, and keep the data element empty;
/// Database and environment handle requirement:
...
public:
	...
	/// \return An iterator sitting on the newly inserted key, or an
	/// invalid one if the key x existed.
	...
	/// This function does not rely on key order, it is doing a multiple
	/// set equal comparison---Two sets A and B are equal if and only if
